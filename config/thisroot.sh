...
if [ -z "${PYTHON_VERSION}" ] ; then
   ...
   append_to_all_libs "_default"
else
   if [ -d "@libdir@/python${PYTHON_VERSION}" ]; then
      if [ -n "${old_rootsys}" ] ; then
         drop_from_all_libs
      fi
   append_to_all_libs "${PYTHON_VERSION}"
   else
      echo ERROR: build with Python version "${PYTHON_VERSION}" not found.
      echo Available versions:
      for version_path in @libdir@/python*
      do
         version_number=${version_path#@libdir@/python}
         if [ ${version_number} = "_default" ]; then
            :
         else
            echo ${version_number}
         fi
...
unset -f drop_from_all_libs
unset -f append_to_all_libs
